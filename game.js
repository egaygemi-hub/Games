...
const LEVELS = [
  ...
  },
  {
    ...
    platforms: [[0, 492, 520, 60], [680, 492, 420, 60], [1280, 492, 360, 60], [1810, 492, 1190, 60], [260, 390, 160, 24], [540, 320, 170, 24], [900, 265, 150, 24], [1190, 350, 180, 24], [1580, 280, 190, 24], [2050, 380, 180, 24], [2390, 315, 220, 24]],
    coins: [[300, 350], [580, 280], [930, 225], [1230, 310], [1625, 240], [1685, 240], [2100, 340], [2460, 275], [2550, 275]],
    powerups: [{ x: 930, y: 226, type: 'star' }, { x: 2440, y: 276, type: 'heart' }],
    enemies: [{ x: 760, y: 456, min: 700, max: 1040 }, { x: 1320, y: 456, min: 1290, max: 1600 }, { x: 2110, y: 344, min: 2060, max: 2200 }, { x: 2520, y: 456, min: 2240, max: 2720 }],
  },
  {
...
function makeGame(level = 0, carry = { score: 0, coins: 0 }) {
  ...
  return {
    ...
    player: { x: data.player.x, y: data.player.y, w: 34, h: 48, vx: 0, vy: 0, onGround: false, health: 100, inv: 0, power: '', powerTime: 0, facing: 1 },
...
function update() {
  ...
  if (p.y > H + 80) hurt(100);
  if (p.inv > 0) p.inv--; if (p.powerTime > 0 && --p.powerTime === 0) p.power = '';
  updateItems(); updateEnemies(); updateBullets();
...
function draw() {
  ...
  drawBackground(cam); drawPlatforms(); drawFlag(); drawItems(); drawEnemies(); drawBullets(); drawPlayer(); drawParticles();
  ctx.restore();
}
...
function drawItems() { for (const c of game.coinsList) if (!c.got) { ctx.fillStyle = '#ffd75a'; ctx.beginPath(); ctx.ellipse(c.x + 11, c.y + 11, 10, 13, 0, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#fff3a7'; ctx.fillRect(c.x + 9, c.y + 3, 4, 16); } for (const p of game.powerups) if (!p.got) { ctx.fillStyle = p.type === 'heart' ? '#ff5c74' : p.type === 'star' ? '#abff5d' : '#ff8d3d'; ctx.fillRect(p.x, p.y, p.w, p.h); ctx.fillStyle = '#fff'; ctx.fillText(p.type === 'heart' ? '♥' : p.type === 'star' ? '★' : 'F', p.x + 7, p.y + 20); } }
